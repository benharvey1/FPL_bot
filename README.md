...
<br>The `GradientBoostingClassifier` class from the `sklearn` library is used for the classification model and `LGBMRegressor` from the `lightgbm` library is used for the regression model. Other models were tested (including random forest classififers/regressors) - see the models folder for details.
