...
def best_team_next_x_gws(df, classifiers, regressors, x, current_team, free_transfers, current_team_value, bank):
    ...
    number_cost_transfers = number_transfers - free_transfers
...
def best_team_next_x_gws_modified(df, classifiers, regressors, x, current_team, free_transfers, current_team_value, bank, min_bench_points=0):
    """Determines best team for the next x gws based on the current team
    """
    predictions_df = pd.DataFrame(predict_next_x_gws(df, classifiers, regressors, x))
